  ...
  return (
    <>
      ...
      <div className="block h-full w-auto md:hidden">
        {!isSideMenuOpen ? (
          <div className="z-20 flex h-full w-[70px] flex-col justify-start bg-light-bgw text-sm shadow-xl dark:bg-[#353c56]/10 lg:hidden">
            <section className="flex h-full flex-col items-center justify-start">
              <section className="flex h-[60px] w-full items-center justify-center">
                <button onClick={() => dispatch(toggleSideMenu())}>
                  <AlignLeft className="h-[24px] w-[24px] text-light-button dark:text-dark-button" />
                </button>
              </section>
              <div className="flex h-auto max-h-screen w-full flex-col items-center justify-center">
                {menuItems.map((menu) => (
                  <ul
                    key={menu.title}
                    className={`font-['apple-system,_system-ui,_BlinkMacSystemFont,_"Segoe_UI",_Roboto,_"Helvetica_Neue",_"Fira_Sans",_Ubuntu,_Oxygen,_"Oxygen_Sans",_Cantarell,_"Droid_Sans",_"Apple_Color_Emoji",_"Segoe_UI_Emoji",_"Segoe_UI_Symbol",_"Lucida_Grande",_Helvetica,_Arial,_sans-serif'], w-auto text-left`}
                  >
